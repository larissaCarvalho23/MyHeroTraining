angular.module('myHeroTraining').factory('cadastroService', function ($http) {
  ...
  var confirmar = function (params) {
    console.log();
    return $http.post('http://localhost:8080/confirm-account', params);
  };
  var recuperar = function (email) {
    console.log();
    return $http.post('http://localhost:8080/forgot-password', email);
  };
  var trocar = function (model) {
    console.log(model);
    return $http.post('http://localhost:8080/confirm-reset', model);
  };
